...
#define BAR "=================================================================="
						...
						// dato che è stato completamento riscritto:
						// su nuova sandbox testalo
						// testa i loop al posto della sleep
						// risolvi problema backup ubuntu
						// fai scaricare direttamente il compilato a automatic main e startup
...
static void on_responsegcc_8Func(GtkDialog *dialog, gint response_id, gpointer user_data){
  	gtk_widget_destroy (GTK_WIDGET (dialog));
  	if(response_id == -5) {
    	system("sudo apt-get install gcc-8-multilib && sudo apt-get install make && ls -la /usr/bin/ | grep -oP \"[\\S]*(gcc|g\\+\\+)(-[a-z]+)*[\\s]\" | xargs bash -c 'for link in ${@:1}; do sudo ln -s -f \"/usr/bin/${link}-${0}\" \"/usr/bin/${link}\"; done' 8");
  	}else printf("NO!\n");
  	system("tput reset");
  	printf("Go to the app--->\n");
}
